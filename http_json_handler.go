...
func MakeJSONRPCHandler(funcMap map[string]*server.RPCFunc, logger log.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if len(responses) > 0 {
			if wErr := server.WriteRPCResponseHTTP(w, responses...); wErr != nil {
...
func JsonParamsToArgs(rpcFunc *server.RPCFunc, raw []byte) ([]reflect.Value, error) {
	const argsOffset = 1
...
func writeListOfEndpoints(w http.ResponseWriter, r *http.Request, funcMap map[string]*server.RPCFunc) {
	noArgNames := []string{}
	argNames := []string{}
